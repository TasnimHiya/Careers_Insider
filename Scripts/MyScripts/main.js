...
function validateLogoUpload(input) {
    ...
    function validatePhone(input) {
        ...
        if (/^\d{11}$/.test(phoneNumber)) {
            ...
            errorSpan.textContent = 'Please enter a valid 11-digit phone number.';
            return false;
        }
    }
...
function validateCompanyName(input) {
    ...
    const stateData = [
        { name: 'Dhaka', cities: ['Dhaka', 'Gazipur', 'Manikganj'] },
        { name: 'Chittagong', cities: ['Chittagong', 'Rangamati', 'Bandarban'] },
        { name: 'Khulna', cities: ['Khulna', 'Jessore', 'Jhenaidah'] },
    ];
            ...
            $.ajax({
                ...
                success: function (data, textStatus, xhr) {
                    if (xhr.status === 200) {
                        ...
                        $("#error-username").text("Username is already taken").removeClass("success-msg").addClass("error-msg");
...
function validateDescription() {
    ...
    if (wordCount < 10) {
        errorSpan.textContent = "Description must contain at least 10 words.";
        return false;
    }
    return true;
}
